...
// $Id: GLG4OpAttenuation.cc,v 1.2 2005/01/03 22:11:27 GLG4sim Exp $
//
...
{
  const double kAngTolerance = 0.000001;// Where did this go? Or where was it originally defined? 
	// An older version of CLHEP perhaps?
  double cos2th=0.0;
  ...
  for (int i=0; i<N_COSTHETA_ENTRIES-1; i++) {
    ...
    do { // find exact root by iterating to convergence
      ...
      cos2th= costh*costh;
    } while (fabs(old_cos2th-cos2th) > kAngTolerance);
...
GLG4OpAttenuation::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
	...
	if ( aMaterialPropertyTable ) {
	   OpScatFracVector = aMaterialPropertyTable->
           ...
           if ( OpScatFracVector )
					 {
             OpScatFrac = OpScatFracVector->GetEnergy (thePhotonMomentum);
           }
	}
