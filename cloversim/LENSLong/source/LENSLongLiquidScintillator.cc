...
LENSLongLiquidScintillator::LENSLongLiquidScintillator(G4String name) : G4VSensitiveDetector(name), m_OpticalPhotonHitsCollection(0), m_GammaHitsCollection(0), m_SingleModuleOnly(false)
{
  // Output info or not.
  m_OutputInfo = true;
...
void LENSLongLiquidScintillator::ConstructLiquidDetector(LENSLongDetectorConstruction *mainDetectorConstructor)
{
  ...
  for (int i = 0; i < number; ++i)
    m_NaI_LogicalVolumes[i]->SetSensitiveDetector(this);

  // Add later if needed. May confuse total summing of NaI modules.
  number = m_Si_LogicalVolumes.size();
  for (int i = 0; i < number; ++i)
    m_Si_LogicalVolumes[i]->SetSensitiveDetector(this);
  ...
  for (int i = 1; i < number; ++i)
    m_Gas_LogicalVolumes[i]->SetVisAttributes(m_GasVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_NaI_LogicalVolumes[i]->SetVisAttributes(m_CellWallVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_PMT_LogicalVolumes[i]->SetVisAttributes(m_PMTGlassVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_Pipe_LogicalVolumes[i]->SetVisAttributes(nextVisAtt);
  ...
  number = m_Si_LogicalVolumes.size();
  for (int i = 0; i < number; ++i)
    m_Si_LogicalVolumes[i]->SetVisAttributes(nextVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_Shield_LogicalVolumes[i]->SetVisAttributes(nextVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_PMT_Shield_LogicalVolumes[i]->SetVisAttributes(nextVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_Wire_LogicalVolumes[i]->SetVisAttributes(nextVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_Cd_LogicalVolumes[i]->SetVisAttributes(nextVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_HDPE_LogicalVolumes[i]->SetVisAttributes(nextVisAtt);
  ...
  for (int i = 0; i < number; ++i)
    m_Glass_LogicalVolumes[i]->SetVisAttributes(nextVisAtt);
...
void LENSLongLiquidScintillator::makeTable()
{
  ...
  const G4double offsetYTable = -0.5 * (m_SodiumIodideDimensions[0][1] +
                                        2.0 * (m_Layer0Thickness + m_Layer1Thickness + m_Layer2Thickness + m_Layer3Thickness)) -
                                halfTableY;
...
void LENSLongLiquidScintillator::makeCloverRhombicuboctahedron()
{
  ...
  if (setAll)
    new G4PVPlacement(0, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 0);
  ...
  if (setAll)
    new G4PVPlacement(rot1, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 2);
  ...
  placement = G4ThreeVector(0.0 * cm, -0.721 * (housingRadius + cloverOffset), -0.403 * (housingRadius + cloverOffset) - zOffset);
  if (setAll)
    new G4PVPlacement(rot5, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 4);
  ...
  if (setAll)
    new G4PVPlacement(rot7, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 6);
  ...
  placement = G4ThreeVector(0.721 * (housingRadius + cloverOffset), 0.0 * cm, -0.403 * (housingRadius + cloverOffset) - zOffset);
  if (setAll)
    new G4PVPlacement(rot9, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 8);
  ...
  placement = G4ThreeVector(0.707 * (housingRadius + cloverOffset), -0.707 * (housingRadius + cloverOffset), 0.0);
  if (setAll)
    new G4PVPlacement(rot13, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 13);

  placement = G4ThreeVector(housingRadius + cloverOffset, 0.0, 0.0);
  if (setAll)
    new G4PVPlacement(rot14, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 14);

  placement = G4ThreeVector(0.707 * (housingRadius + cloverOffset), 0.707 * (housingRadius + cloverOffset), 0.0);
  if (setAll)
    new G4PVPlacement(rot15, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 15);

  placement = G4ThreeVector(0.0, housingRadius + cloverOffset, 0.0);
  if (setAll)
    new G4PVPlacement(rot16, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 16);

  placement = G4ThreeVector(-0.707 * (housingRadius + cloverOffset), 0.707 * (housingRadius + cloverOffset), 0.0);
  if (setAll)
    new G4PVPlacement(rot17, placement, logVol, "SingleModule_Volume", m_Gas_LogicalVolumes[0], 0, 17);
}
...
{ // This function assumes the it is a PMT volume that is hit. Nothing else absorbs optical photon and this function
  ...
  if (fabs(momentumDir[2]) < M_SQRT1_2) // angle greater than 45 degrees from normal
  {
    ...
    if (random > -4.0 * angle / M_PI + 2.0)
      return true; // Reject the counting...
  }
