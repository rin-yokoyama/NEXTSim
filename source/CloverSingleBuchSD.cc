...
CloverSingleBuchSD::CloverSingleBuchSD(G4String name)
  :G4VSensitiveDetector(name), clov_ID(-1), crys_ID(-1), HCID(-1)
{
  HC_name = "CloverSingleBuch";
...
void CloverSingleBuchSD::Initialize(G4HCofThisEvent* evnt)
{
  hitsCollection = new CloverSingleBuchHitsCollection(SensitiveDetectorName, HC_name);
...
G4bool CloverSingleBuchSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
...
*/
  ...
  CloverSingleBuchHit* newHit = new CloverSingleBuchHit();
...
void CloverSingleBuchSD::EndOfEvent(G4HCofThisEvent*)
{;}
