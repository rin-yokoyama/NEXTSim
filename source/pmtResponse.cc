...
bool spectralResponse::load(const std::string &fname)
{
	...
	{ // Root file. Expects a TGraph named "spec"
...
{
	double efficiency = 1;
	if (useSpectralResponse)
	{ // Compute the quantum efficiency of the PMT for this wavelength.
	...
	if (timeSpread > 0)
	{ // Smear the time offset based on the photo-electron transit time spread.
...
void pmtResponse::getRawPulse(std::vector<double> &rawPulse) const
{
	...
	for (size_t index = 0; index < pulseLength; index++)
	{
		...
		if (rawPulse[index] < prevAmp && rawPulse[index] < 1)
		{ // Amplitude is less than one ADC bin
			rawPulse[index] = 0;
			break;
		}
...
double pmtResponse::integratePulse(const size_t &start_, const size_t &stop_)
{
	...
	double qdc = 0.0;
	for (size_t i = start_ + 1; i < stop; i++)
...
{
	if (functionType == EXPO)
	{ // Standard single photon response function.
		if (t - dt <= 0)
			return 0;
		return gain * (1 / (falltime - risetime)) * (std::exp(-(t - dt) / falltime) - std::exp(-(t - dt) / risetime));
	}
	else if (functionType == VANDLE)
	{ // "Vandle" function. The risetime is BETA and the decay time is GAMMA.
		if (t - dt <= 0)
			return 0;
		return (100 * gain * std::exp((dt - t) * risetime) * (1 - std::exp(-std::pow((t - dt) * falltime, 4))));
	}
	else if (functionType == GAUSS)
	{ // Normalized gaussian function. The risetime is sigma and the decay time is not used.
