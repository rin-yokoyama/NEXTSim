...
const double KINETIC_ENERGY_THRESHOLD = 0.01; // MeV
...
bool nDetRunAction::processDetector(nDetDetector *det, int detNum)
{
	...
	debugData.photonDetComX[0] = centerL.getX();
	debugData.photonDetComX[1] = centerR.getX();
	debugData.photonDetComY[0] = centerL.getY();
	debugData.photonDetComY[1] = centerR.getY();
...
void nDetRunAction::process()
{
	...
	{ // Start triggered mode
		double startTime;
		if (processStartDetector(startDetector, startTime))
		{ // Check for valid start signal
			for (std::vector<nDetDetector>::iterator iter = userDetectors.begin(); iter != userDetectors.end(); iter++)
			{
				// Skip the start detector because we already processed it
	...
	//+ G4INCL::Random::gauss(0.278969957)
...
bool nDetRunAction::scatterEvent()
{
	...
	if (evtData.nScatters++ == 0)
	{ // First scatter event (CAREFUL! These are in reverse order).
...
bool nDetRunAction::scatterNeutron(const G4Step *step)
{
	...
	if (dE > 0)
	{
		...
		if (track->GetKineticEnergy() < KINETIC_ENERGY_THRESHOLD)
		{ // Check particle kinematic energy.
