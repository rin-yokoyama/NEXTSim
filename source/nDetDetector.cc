...
{
	if (!this->checkCopyNumber(copyNum))
		return false;
...
{
	return (new G4PVPlacement(rot, pos, volume, (name.empty() ? volume->GetName() : name), assembly_logV, false, parentCopyNum, checkOverlaps));
}
...
{
	if (thickness > 0)
	{ // Build the light diffusers (if needed)
...
{
	if (thickness > 0)
	{ // Build the light guides (if needed)
